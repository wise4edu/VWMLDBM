...
function display_db_check(){
	...
	echo "<br><b> * You should have Database Account Information of your Web Server.<br>";
	echo "<table>";
	echo "<tr>
			<td>Database Host</td>
			<td>localhost</td>
...
// DB name
	...
	else if(substr($WMALL['DB'],0,9)=="_INSTALL_") 
		$val="";
	else $val=$WMALL['DB'];	
	echo "<tr>
			<td>Database Name</td>
...
// DB User
	...
	else if(substr($WMALL['DB_user'],0,9)=="_INSTALL_") 
		$val="";
	else $val=$WMALL['DB_user'];	
	echo "<tr>
			<td>Database User</td>
...
// DB Password
	...
	echo "<tr>
		...
		<td><input type=text name='dbpasswd' value='$val' $readonly onchange='document.form1.go_button.disabled=true;'></td>
	  </tr>
	";

// Table prefix
	if($_POST['TB_prefix']) $val=$_POST['TB_prefix'];
	else if(substr($WMALL['TB_prefix'],0,9)=="_INSTALL_") 
		$val="";
	else $val=$WMALL['TB_prefix'];	
	echo "<tr>
			<td>Table Prefix</td>
	...
	if($_POST['operation']=='db_check'){
		if($db_con_ok) {			
			...
			if($tb_pre_ok) install\display_ok_msg("Table Prefix is available!");
			else install\display_error("Table Prefix is NOT available. Choose another one!");
		}
