...
<script src='../js/rmd.js?nocache=23'></script>
<script>
...
if($_POST['operation']=='SAVE_TABLE_POSITION'){ // When user submit "Save table positions"
	...
	mysqli_query($conn,"delete from {$DTB_PRE}vwmldbm_rmd_tb_loc where inst='$inst_no'");
	for($i=0;$i<count($_POST['table_no']);$i++){
		...
		if($_POST['table_expanded'][$i]!=NULL) $sql="insert into {$DTB_PRE}vwmldbm_rmd_tb_loc (inst,tb_no,x_pos,y_pos,expanded_yn,zindex) 
			values('$inst_no','$tb_no','$table_x_pos','$table_y_pos','$expanded_yn','$zidx')";
		else $sql="insert into {$DTB_PRE}vwmldbm_rmd_tb_loc (inst,tb_no,x_pos,y_pos,zindex) 
...
echo " &nbsp; <input type=checkbox id='vwmldbm_check' checked onClick=\"show_hide_vwmldbm(this);\"> Show VWMLDBM Tables ";
...
$num_table_printed_so_far=RMD\show_DB_ERD($inst_no); // Show all the tables
