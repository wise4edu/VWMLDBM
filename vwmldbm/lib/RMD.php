...
function show_table($db=null,$tb=null,$table_id=null,$inst=null){  // show the specified table of the DB
	...
	$sql="select t.no from {$DTB_PRE}vwmldbm_tb t, {$DTB_PRE}vwmldbm_db d where t.db=d.no and t.name='$tb' and d.name='$db'";
	$res_ss=mysqli_query($conn,$sql);
	if($res_ss) $rs_ss=mysqli_fetch_array($res_ss);
	
	$sql="select expanded_yn from {$DTB_PRE}vwmldbm_rmd_tb_loc where tb_no='".$rs_ss['no']."'";
...
// find the table prefix	
	...
	$num_DB_printed_so_far=0;
	$sql="select * from {$DTB_PRE}vwmldbm_tb where DB='{$rs['no']}'";
	...
	if($res2) while($rs2=mysqli_fetch_array($res2)){ // print each table's div
		...
		$sql="select zindex from {$DTB_PRE}vwmldbm_rmd_tb_loc where inst='$inst' and tb_no='".$rs2['no']."'";
...
if(substr($trs2name,-9)=='vwmldbm_inst') $sql="select * from ".$rs['name'].".".$rs2['name']." where no='$inst' ";
		...
		$sql="select * from {$DTB_PRE}vwmldbm_rmd_tb_loc where inst='$inst' and tb_no='".$rs2['no']."'";
		$res_RMD=mysqli_query($conn,$sql);
		if($res_RMD) $rs_ERD=mysqli_fetch_array($res_RMD);
...
// get the no of DB
	global $conn,$DB,$DTB_PRE;
	$res_n=mysqli_query($conn,"select no from {$DTB_PRE}vwmldbm_db");
	if($res_n){
